...
import { sendMailFromApp } from "@/lib/send-mail";
...
export const approveParticipant = async (id: string) => {
  try {
    ...
    if (participant.isApproved) {
      return { error: "Participant have already been approved" };
    }

    await db.launchCampaignParticipants.update({
      where: { id },
      data: { isApproved: true },
    });

    await sendMailFromApp({
